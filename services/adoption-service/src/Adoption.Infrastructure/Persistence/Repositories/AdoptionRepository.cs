public sealed class AdoptionRepository(AdoptionDbContext context) : IAdoptionRepository
{
    ...
    public async Task<IEnumerable<AdoptionRequest>> GetByPetIdAsync(Guid petId, CancellationToken ct = default)
        => await context.AdoptionRequests.Where(x => x.PetId == petId && !x.IsDeleted).ToListAsync(ct);
